...
const apiDispatcher = route('api/*', './api-dispatcher.ts');
const sitemap = route('sitemap.xml', './sitemap.ts');
const robots = route('robots.txt', './robots.ts');
const llmsTxt = route('llms.txt', './llms-txt.ts');
const llmsFullTxt = route('llms-full.txt', './llms-full-txt.ts');
const notFound = route('*', './not-found.tsx');
const routes = [...generateRoutes(tree), sitemap, robots, llmsTxt, llmsFullTxt, apiDispatcher, notFound];
